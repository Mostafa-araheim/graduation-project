...
@Component({
  ...
  template: `
    <div class="min-h-screen flex">
      ...
      >
        <div
          class="absolute -top-20 -left-20 w-72 h-72 rounded-full bg-white/10 blur-sm"
        ></div>
        <div
          class="absolute top-1/4 right-10 w-48 h-48 rounded-full bg-white/5"
        ></div>
        <div
          class="absolute bottom-20 left-16 w-36 h-36 rounded-full bg-white/[0.07]"
        ></div>
        <div
          class="absolute -bottom-10 -right-10 w-64 h-64 rounded-full bg-white/[0.04]"
        ></div>
        <div
          class="absolute top-10 left-1/3 w-20 h-20 rounded-full bg-white/10"
        ></div>
        <div
          class="absolute bottom-1/3 right-1/4 w-14 h-14 rounded-full bg-accent-400/20"
        ></div>

        <div class="relative z-10 max-w-md text-center lg:text-left">
          <div
            class="flex items-center gap-3 mb-6 justify-center lg:justify-start"
          >
            <div
          ...
          <div class="space-y-4">
            <div class="flex items-center gap-3">
              <div
                class="w-8 h-8 rounded-lg bg-white/15 flex items-center justify-center flex-shrink-0"
              >
                <svg
                  class="w-4 h-4 text-accent-300"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  stroke-width="2.5"
              ...
              <span class="text-primary-100 text-sm font-medium"
                >Create your owner workspace in minutes</span
              >
            </div>
            <div class="flex items-center gap-3">
              <div
              ...
              <span class="text-primary-100 text-sm font-medium"
                >Verify securely with a one-time code</span
              >
            </div>
            <div class="flex items-center gap-3">
              <div
              ...
              <span class="text-primary-100 text-sm font-medium"
                >Start managing pharmacies from one dashboard</span
              >
            </div>
          </div>
        </div>
      ...
      <div
        class="w-full lg:w-1/2 flex items-center justify-center bg-slate-50 p-6 sm:p-8"
      >
        <div class="w-full max-w-md">
          <div class="lg:hidden flex items-center gap-2 mb-8 justify-center">
            <div
              class="w-10 h-10 rounded-xl bg-primary-600 flex items-center justify-center"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
          ...
          <div class="bg-white rounded-2xl shadow-card p-8">
            <div [class.hidden]="step() !== 1">
              <div class="mb-8">
                <h2 class="font-heading text-3xl font-bold text-gray-900 mb-2">
                  Create Account
                </h2>
              ...
              <form [formGroup]="signupForm" (ngSubmit)="onSignupSubmit()">
                ...
                @if (errorMessage() && step() === 1) {
                  <div
                    class="mb-4 p-3 rounded-xl bg-red-50 border border-red-100 text-red-600 text-sm flex items-center gap-2"
                  >
                    <svg
                  ...
                  @if (
                    ...
                    >
                      ...
                      @if (otpForm.controls.code.errors['required']) {
                        ...
                        otpForm.controls.code.errors['minlength'] ||
                        otpForm.controls.code.errors['maxlength']
                      ) {
          ...
          <p class="mt-3 text-center text-xs text-gray-400">
            By continuing, you agree to PharmaOwner's Terms of Service and
            Privacy Policy.
          </p>
        </div>
      </div>
...
export class SignupComponent implements OnInit {
  ...
  onSignupSubmit(): void {
    ...
    const nameValue = this.signupForm.controls.name.value!;
    this.errorMessage.set('');
    this.loading.set(true);

    this.authService.signupStart(emailValue, nameValue).subscribe({
      next: (response) => {
        if (response.data) {
          console.log(response);
          this.signupId.set(response.data.signupId);
  ...
  onOtpSubmit(): void {
    ...
    this.authService.signupVerify(this.signupId(), code).subscribe({
      next: () => {
        this.loading.set(false);
  ...
  resendCode(): void {
    ...
    this.authService.signupStart(this.email(), this.name()).subscribe({
      next: (response) => {
        if (response.data) {
          this.signupId.set(response.data.signupId);
          this.countdown.set(180);
